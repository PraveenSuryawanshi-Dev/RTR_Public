...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
	...
	TCHAR szAppName[] = TEXT("Interleaved VCNT Using Structures");
	HDC hdc = NULL;
	HGLRC hglrc = NULL;
...
void IntializeOpenGL(HWND hwnd, HDC* hdc, HGLRC* hglrc)
{
	...
	*/
	...
	for (int i = 0; i < 24; i++)
	{
		...
		glCube[i].vertices[0] = cubeVCNT[i][j++];
		glCube[i].vertices[1] = cubeVCNT[i][j++];
		glCube[i].vertices[2] = cubeVCNT[i][j++];

		glCube[i].color[0] = cubeVCNT[i][j++];
		glCube[i].color[1] = cubeVCNT[i][j++];
		glCube[i].color[2] = cubeVCNT[i][j++];

		glCube[i].normals[0] = cubeVCNT[i][j++];
		glCube[i].normals[1] = cubeVCNT[i][j++];
		glCube[i].normals[2] = cubeVCNT[i][j++];

		glCube[i].tex[0] = cubeVCNT[i][j++];
		glCube[i].tex[1] = cubeVCNT[i][j++];
	}
	...
	glBufferData(GL_ARRAY_BUFFER,sizeof(glCube), &glCube, GL_STATIC_DRAW);
