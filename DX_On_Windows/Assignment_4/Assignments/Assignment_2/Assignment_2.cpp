...
ID3D11ShaderResourceView *pID3D11ShaderResourceViewQuad = NULL;
...
HRESULT Initialize()
{
	...
	hr = LoadD3DTexture(L"Smiley.bmp", &pID3D11ShaderResourceViewQuad);
	if (FAILED(hr))
	{
...
void RenderQuad()
{
	...
	//BIND TEXTURE
	pID3D11DeviceContext->PSSetShaderResources(0, 1, &pID3D11ShaderResourceViewQuad);
...
void UnInitialize()
{
		SAFE_RELEASE(pID3D11ShaderResourceViewQuad)
		SAFE_RELEASE(pID3D11DepthStencilView)
