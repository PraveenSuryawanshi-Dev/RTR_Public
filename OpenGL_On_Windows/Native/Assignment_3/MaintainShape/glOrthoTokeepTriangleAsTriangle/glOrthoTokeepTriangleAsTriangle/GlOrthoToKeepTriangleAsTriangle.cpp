...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
	...
	TCHAR szAppName[] = TEXT("GLOrtho TO KEEP TRIANGLE AS TRIANGLE APPLCATION USING OPENGL");
	HDC hdc = NULL;
	HGLRC hglrc = NULL;
...
void Resize(int width, int height)
{
	...
	if (width <= height)
	{
		glOrtho(-50.0f, 50.0f, -50.0f * (GLfloat)height / (GLfloat)width, 50.0f * (GLfloat)height / (GLfloat)width, -50.0f, 50.0f); // volume 100
	}
	else
	{
		glOrtho(-50.0f * (GLfloat)width / (GLfloat)height, 50.0f * (GLfloat)width / (GLfloat)height, -50.0f, 50.0f, -50.0f, 50.0f); // volume 100
	}
