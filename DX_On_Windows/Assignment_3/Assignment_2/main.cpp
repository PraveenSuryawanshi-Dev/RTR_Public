...
{	
	...
	"{"\
		"vs_out vsout;"\
		"vsout.position = mul(worldProjectionMatrix,pos);"\
		"vsout.color = color;"\
		"return (vsout);"\
	"}";
	...
	hr = gpID3D11Device->CreateInputLayout(inputElementDesc,2,pID3DBlob_vs->GetBufferPointer(),pID3DBlob_vs->GetBufferSize(),&gpID3D11InputLayout);
	if(FAILED(hr))
	{
	...
	bufferDesc.BindFlags =D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = gpID3D11Device->CreateBuffer(&bufferDesc,NULL,&gpID3D11Buffer_ColorBuffer);
	if(FAILED(hr))
	{
		SAFE_LOG("CreateBuffer function failed.")
		return hr;
	...
	//copy vertices to vbo 
	ZeroMemory(&mappedSubresource,sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer,NULL,D3D11_MAP_WRITE_DISCARD,NULL,&mappedSubresource);
...
HRESULT Resize(int width ,int height)
{
	...
	gPerspectiveProjectionMatrix = XMMatrixPerspectiveFovLH(45.05f, (FLOAT)width / (FLOAT)height, 0.1f, 100.0f);
	return S_OK;
}
...
void Render()
{
	...
	worldMatrix = XMMatrixTranslation(0.0f,0.3f, 1.0f);
