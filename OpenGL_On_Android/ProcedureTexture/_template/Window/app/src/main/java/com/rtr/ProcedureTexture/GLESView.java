...
{ 
   ...
   final  int TEX_HEIGHT = 64;
	...
	private int[] texture_checkboard = new int[1];
	//private int gCheckboard_pixels[] = new int[TEX_WIDTH * TEX_HEIGHT * 4 ];
	private byte [][][]gCheckboard_pixels = new byte[TEX_WIDTH][TEX_HEIGHT][4];
	...
	int columns = TEX_HEIGHT;
	
	int depth = 4;
	
	byte[] result1D = new byte[rows * columns * depth];
	...
	private void CreateCheckboardTexture()
	{
		System.out.println("PRAVEEN : IN CreateCheckboardTexture ");
		...
		int	c = 0;
			...
			for (int j = 0; j < rows; j++)
			{
				for (int k = 0; k < columns; k++) 
				{
					...
					result1D[0 + depth*(j + rows*(k))] = (byte)c;
					result1D[1 + depth*(j + rows*(k))] = (byte)c;
					result1D[2 + depth*(j + rows*(k))] = (byte)c;
					result1D[3 + depth*(j + rows*(k))] = (byte)255;
				}
			}	

		
		
		System.out.println("PRAVEEN : OUT CreateCheckboardTexture ");
	}
	
	 private void loadGLTexture()
    {	
		System.out.println("PRAVEEN : IN  loadGLTexture ");
		 //Get Bitmap Factory Options intance
        //BitmapFactory.Options options = new BitmapFactory.Options();
        //options.inScaled = false;
		
        //Bitmap bitmap = null;//Bitmap.createBitmap(gCheckboard_pixels[0], TEX_WIDTH,TEX_HEIGHT, options);

        
        // create a texture object to apply to model
        GLES30.glGenTextures(1, texture_checkboard, 0);
        ...
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D,texture_checkboard[0]);
		...
		/*int rows = gCheckboard_pixels.length;
		int columns = gCheckboard_pixels[0].length;
		int depth = gCheckboard_pixels[0][0].length;
		byte[] result = new byte[rows * columns * depth];
		...
		for (int i = 0; i < depth; i++) 
		{
			for (int j = 0; j < rows; j++)
			{
				for (int k = 0; k < columns; k++) 
				{
					result[i + depth*(j + rows*(k))] = gCheckboard_pixels[j][k][i];
				}
			}	
		}*/
		...
		ByteBuffer byteBuffer=ByteBuffer.allocateDirect(result1D.length);
        byteBuffer.order(ByteOrder.nativeOrder());
		byteBuffer.put(result1D);
        byteBuffer.position(0);
		
		GLES30.glTexImage2D(GLES30.GL_TEXTURE_2D, 0/*LOD*/, GLES30.GL_RGBA, TEX_WIDTH, TEX_HEIGHT, 0/*border width*/, GLES30.GL_RGBA,GLES30.GL_UNSIGNED_BYTE, byteBuffer);
        
        // generate mipmap
        GLES30.glGenerateMipmap(GLES30.GL_TEXTURE_2D);
		
		System.out.println("PRAVEEN : OUT  loadGLTexture ");
	...
	public void draw()
	{
		...
		//
		final float rectangleVertices_front[] =
		{
			0.0f, 1.0f, 0.0f,
			-2.0f, 1.0f, 0.0f,
			-2.0f, -1.0f, 0.0f,
		...
		final float rectangleCTexcocords_front[] =
		{
			1.0f, 1.0f, 
			0.0f, 1.0f, 
			0.0f, 0.0f, 
		...
		ByteBuffer byteBuffer=ByteBuffer.allocateDirect(rectangleVertices_front.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        
		FloatBuffer verticesBuffer=byteBuffer.asFloatBuffer();
        verticesBuffer.put(rectangleVertices_front);
        verticesBuffer.position(0);
		
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,
							rectangleVertices_front.length * 4,
                            verticesBuffer,
		...
		byteBuffer=ByteBuffer.allocateDirect(rectangleCTexcocords_front.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        
		verticesBuffer=byteBuffer.asFloatBuffer();
        verticesBuffer.put(rectangleCTexcocords_front);
        verticesBuffer.position(0);
		
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,
                            rectangleCTexcocords_front.length * 4,
                            verticesBuffer,
                            GLES30.GL_STATIC_DRAW);
		...
		//-----------------------------------------------------------------------------------
		//
		final float rectangleVertices_deviated[] =
		{
		...
		final float rectangleCTexcocords_deviated[] =
		{
			1.0f, 1.0f,
			0.0f, 1.0f,
			0.0f, 0.0f,
		...
		byteBuffer=ByteBuffer.allocateDirect(rectangleVertices_deviated.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        
		verticesBuffer=byteBuffer.asFloatBuffer();
        verticesBuffer.put(rectangleVertices_deviated);
        verticesBuffer.position(0);
		
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,
							rectangleVertices_deviated.length * 4,
                            verticesBuffer,
                            GLES30.GL_STATIC_DRAW);
		...
		byteBuffer=ByteBuffer.allocateDirect(rectangleCTexcocords_deviated.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        
		verticesBuffer=byteBuffer.asFloatBuffer();
        verticesBuffer.put(rectangleCTexcocords_deviated);
        verticesBuffer.position(0);
		
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,
                            rectangleCTexcocords_deviated.length * 4,
                            verticesBuffer,
                            GLES30.GL_STATIC_DRAW);
