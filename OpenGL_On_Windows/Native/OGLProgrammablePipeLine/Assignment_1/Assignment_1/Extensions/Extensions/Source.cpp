...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
	...
	TCHAR szAppName[] = TEXT("EXTENSIONS SUPPORTED BY DRIVER");
	HDC hdc = NULL;
	HGLRC hglrc = NULL;
...
void IntializeOpenGL(HWND hwnd, HDC* hdc, HGLRC* hglrc)
{
	...
	*hglrc = wglCreateContext(*hdc);
	...
	FILE *fp = fopen("EXTENSION_LOG.txt", "w");

	if (fp == NULL)
	{
		MessageBox(hwnd, TEXT("ERROR TO fopen"), TEXT("ERROR"), MB_ICONERROR);
		return;
	}
	...
	for (int i = 0; i < num; i++)
	{	 
	 const char *string = (const char *)	glGetStringi(GL_EXTENSIONS, i);
	 if (string == NULL)
		 continue;
	...
	fclose(fp);
	fp = nullptr;
	MessageBox(hwnd, TEXT("FILE CLOSED SUCCESFULLY !!! EXTENSIONS SAVED."), TEXT("ERROR"), MB_ICONERROR);
