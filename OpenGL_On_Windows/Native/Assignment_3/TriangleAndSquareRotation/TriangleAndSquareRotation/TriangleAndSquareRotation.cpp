...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
	...
	TCHAR szAppName[] = TEXT("TRIANGLE AND SQUARE ROTATION APPLCATION USING OPENGL");
	HDC hdc = NULL;
	HGLRC hglrc = NULL;
...
void Render(HDC hdc)
{
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(1.50f,0.0f,-6.0f);
	glRotatef(angleRect, -1.0f, 0.0f, 0.0f); // rotation around X axis
	DrawRectangle();
...
void Update()
{
	...
	if (angleRect >= 360) angleRect = 0.0f;
	angleRect = angleRect + 0.5f;
}
