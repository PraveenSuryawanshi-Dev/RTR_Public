...
GLuint gvao_pyramid_texture;

GLuint gVao_cube_position;
GLuint gvao_cube_texture;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
	...
	TCHAR szAppName[] = TEXT("Rotate 3D TEXTURE PYRAMID and 3D TEXTURE CUBE PERSPECTIVE OGL");
	HDC hdc = NULL;
	HGLRC hglrc = NULL;
...
void IntializeOpenGL(HWND hwnd, HDC* hdc, HGLRC* hglrc)
{
	...
	*/
	...
	const GLfloat pyramidCTextcocords[]=
	{ 
		//FRONT FACE
		0.5f, 1.0f, 
	    0.0f , 0.0f,
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(pyramidCTextcocords), &pyramidCTextcocords, GL_STATIC_DRAW);
	...
	//LOAD TEXTURES 
	LoadGLTexture(&gTexture_kundali, MAKEINTRESOURCE(IDIBITMAP_KUNDALI));
	LoadGLTexture(&gTexture_stone, MAKEINTRESOURCE(IDIBITMAP_STONE));
	//
...
void UnIntializeOpenGL(HWND hwnd, HDC* hdc, HGLRC* hglrc)
{
	...
	*hdc = NULL;
	...
	if (gvao_cube_texture)
	{
		glDeleteVertexArrays(1, &gvao_cube_texture);
		gvao_cube_texture = 0;
	}
	...
	if (gvao_pyramid_texture)
	{
		glDeleteVertexArrays(1, &gvao_pyramid_texture);
		gvao_pyramid_texture = 0;
	}
