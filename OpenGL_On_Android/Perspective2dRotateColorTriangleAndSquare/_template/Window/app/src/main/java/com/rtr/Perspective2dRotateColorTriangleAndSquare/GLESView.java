package com.rtr.Perspective2dRotateColorTriangleAndSquare;
...
{ 
	...
	private int [] vao_triangle_position = new int[1];
	private int [] vao_triangle_color = new int[1];
	private float angle_triangle;
	

	
	private int [] vao_square_position = new int[1];
	private int [] vao_square_color = new int[1];
	...
	private void Update()
	{
		if (angle_triangle >= 360.0f) angle_triangle = 0.0f;
			angle_triangle = angle_triangle + 0.5f;

		if (angle_square >= 360) angle_square = 0.0f;
			angle_square = angle_square + 0.5f;
	}
	...
	// PRE - BINDING OF Vcolor attribute	
    GLES30.glBindAttribLocation(shaderProgramObject,GLESMacros.VDG_ATTRIBUTE_COLOR,"vColor");
	...
	final float squareColor[] = new float[]
	{
		...
		/*TRIANGLE POSITION*/
		GLES30.glGenVertexArrays(1,vao_triangle_position,0);
		GLES30.glBindVertexArray(vao_triangle_position[0]);
        ...
        verticesBuffer.put(triangleColor);
        verticesBuffer.position(0);
		...
		/*POSITION*/
		GLES30.glGenVertexArrays(1,vao_square_position,0);
		GLES30.glBindVertexArray(vao_square_position[0]);
		...
		verticesBuffer=byteBuffer.asFloatBuffer();
        verticesBuffer.put(squareColor);
        verticesBuffer.position(0);
		
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,
                            squareColor.length * 4,
                            verticesBuffer,
                            GLES30.GL_STATIC_DRAW);
	...
	{
        // DELETE vao
        if(vao_square_position[0] != 0)
        {
            GLES30.glDeleteVertexArrays(1, vao_square_position, 0);
            vao_square_position[0]=0;
        }
		 if(vao_square_color[0] != 0)
        {
            GLES30.glDeleteVertexArrays(1, vao_square_color, 0);
            vao_square_color[0]=0;
        }
		
		if(vao_triangle_position[0] != 0)
        {
            GLES30.glDeleteVertexArrays(1, vao_triangle_position, 0);
            vao_triangle_position[0]=0;
        }
		if(vao_triangle_color[0] != 0)
        {
            GLES30.glDeleteVertexArrays(1, vao_triangle_color, 0);
            vao_triangle_color[0]=0;
        }
